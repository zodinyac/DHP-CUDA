 ...
 * * Lomonosov: make
 * Run. 
 * * Lomonosov: sbatch -n<proc_count> -p gputest --ntasks-per-node=1 -o stdout_cuda_<proc_count>_<nodes1>_<nodes2>.txt ompi ./dhp-cuda <nodes1> <nodes2>
 */
...
int main(int argc, char *argv[])
{
    if (argc < 3) {
usage_label:
        fprintf(stderr, "Usage: dhp <nodes number 1> <nodes number 2>.\n");
        return 1;
    }
	...
	sp = 0.0;
	cuda_calc_product(cuda_res_vect, cuda_res_vect, &sp, indexes);
	ttau = sp * h[X] * h[Y];
	MPI_Allreduce(&ttau, &tau, 1, MPI_DOUBLE, MPI_SUM, COMM_GRID);
    
    // the value of product sp = (Ar(k),r(k)) is calculating...
	sp = 0.0;
	cuda_calc_Aproduct(cuda_res_vect, cuda_res_vect, &sp, indexes, h);
	tsp = sp * h[X] * h[Y];
	MPI_Allreduce(&tsp, &sp, 1, MPI_DOUBLE, MPI_SUM, COMM_GRID);
	tau = tau / sp;

    // the x(k+1) is calculating...
    cuda_calc_it0_solution_vector(cuda_sol_vect, cuda_res_vect, indexes, tau);
    ...
    do {
        ...
        alpha = 0.0;
        cuda_calc_Aproduct(cuda_res_vect, cuda_basis_vect, &alpha, indexes, h);
        ...
        cuda_calc_basis_vector(cuda_basis_vect, cuda_res_vect, indexes, alpha);
        
        // the value of product (r(k),g(k)) is being calculated...
        tau = 0.0;
        cuda_calc_product(cuda_res_vect, cuda_basis_vect, &tau, indexes);
        ttau = tau * h[X] * h[Y];
        MPI_Allreduce(&ttau, &tau, 1, MPI_DOUBLE, MPI_SUM, COMM_GRID);
        ...
        sp = 0.0;
        cuda_calc_Aproduct(cuda_basis_vect, cuda_basis_vect, &sp, indexes, h);
        tsp = sp * h[X] * h[Y];
        MPI_Allreduce(&tsp, &sp, 1, MPI_DOUBLE, MPI_SUM, COMM_GRID);
        tau = tau / sp;
        ...
        err = 0.0;
        cuda_calc_itn_solution_vector(cuda_sol_vect, cuda_basis_vect, &err, indexes, tau);
    ...
    // compute error
	terr = 0.0;
    cuda_calc_error(cuda_sol_vect, &terr, indexes, h);
...
end_label:
    ...
    cuda_delete_array(sol_vect);
    cuda_delete_array(basis_vect);
